      ...
      <div className="bg-white w-full max-w-2xl rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto flex-1">
            ...
            <tbody className="text-lg">
              {order.items.map((item, idx) => (
                <tr key={idx} className="border-b border-gray-100">
                  <td className="py-3 font-bold text-gray-800">{item.qty}</td>
          ...
          <div className="mt-6 flex justify-end items-center gap-4 pt-4 border-t-2 border-gray-100">
             <span className="text-gray-500 text-lg uppercase font-bold">Total Comanda:</span>
    ...
    <div className="h-screen flex flex-col bg-gray-50">
      ...
      <div className="flex-1 overflow-auto p-4 pb-48 md:pb-32">
        {activeTab === 'menu' ? (
          <>
            <div className="mb-4">
              <input type="number" value={table} onChange={(e) => setTable(e.target.value)} placeholder="Nº Mesa" className="w-full p-3 border rounded-lg text-lg outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div className="grid grid-cols-2 gap-3 mb-6">
              {MENU_ITEMS.map(item => (
                <button key={item.id} onClick={() => addToCart(item)} className="bg-white p-3 rounded-lg shadow border active:bg-blue-50 text-left">
              ...
              <div key={order.id} className="bg-white p-4 rounded-lg shadow border">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span className={`px-2 py-1 rounded text-xs font-bold uppercase 
                    ${order.status === ORDER_STATUS.READY ? 'bg-green-100 text-green-800 animate-pulse' : 'bg-gray-100'}
                    ${order.status === ORDER_STATUS.DELIVERED ? 'bg-gray-200 text-gray-500' : ''}
                  `}>
                    {order.status}
                  </span>
                </div>
                <ul className="text-sm text-gray-600 mb-3">{order.items.map((it, idx) => <li key={idx}>{it.qty}x {it.name}</li>)}</ul>
                <div className="flex justify-end gap-2">
                  ...
                  {order.status === ORDER_STATUS.DELIVERED && (
                    <span className="text-gray-400 text-sm italic py-2">Esperando cobro en caja...</span>
                  )}
                </div>
              </div>
            ))}
      ...
      {activeTab === 'menu' && cart.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t shadow p-4 z-50">
          ...
          <div className="flex gap-4 items-center">
            ...
            <button onClick={sendOrder} disabled={submitting} className="flex-1 bg-green-600 text-white py-3 rounded-lg font-bold shadow disabled:opacity-50">
